...
class MoviesList extends React.Component {
  ...
  render() {
      if (this.props.searchTerm !== '') {
        const foundedMovies = this.findMovie(this.props.movies, this.props.searchTerm);
        console.log(foundedMovies);
        return (
          <section className="movies">
          {foundedMovies.map(m => <SingleMovie movie={m} key={m.id} genresArr={this.props.genresArr} />)}
          </section>
        )
      } else {
        return (
